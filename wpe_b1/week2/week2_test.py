...
def test_myrange2(first, second, third, output):
    if third:
        assert myrange2.myrange2(first, second, third) == output
    else:
        assert myrange2.myrange2(first, second) == output

def test_myrange3_is_a_generator():
    output = myrange3.myrange3(10)
...
def test_myrange3(first, second, third, output):
    if third:
        assert list(myrange3.myrange3(first, second, third)) == output
    else:
        assert list(myrange3.myrange3(first, second)) == output